...
void Drag::advance(Bird* bird)
{
    Velocity temp(bird->getVelocity());
    temp *= 0.995;
    *bird = temp;
}
...
void AntiGrav::advance(Bird* bird)
{
    Velocity temp(bird->getVelocity());
    temp.addDy(0.05);
    *bird = temp;
...
void Random::advance(Bird* bird)
{
    if (randomInt(0, 15) == 0)
    {
        Velocity temp(bird->getVelocity());
        temp.addDy(randomFloat(-1.5, 1.5));
        temp.addDx(randomFloat(-1.5, 1.5));
        *bird = temp;
    }
}
